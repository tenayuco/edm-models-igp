...
tic()
out_cv <- LV_map_state_space_cross_validation(N, theta_v = seq(0, 5, 0.05), p = 0.1)
out_cv_t <- LV_map_time_cross_validation(N, theta_v = seq(0, 5, 0.05), p = 0.1)
toc()

# run the LV-map for the best theta value
tic()
out <- LV_map(N, theta = out_cv$theta_o)
out_t <- LV_map(N, theta = out_cv_t$theta_o, kernel = "time")
toc()
