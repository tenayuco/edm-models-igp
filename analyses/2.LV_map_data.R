...
names(DATA_USED) <- c("replicate", "R", "N", "P", "time")
...
DATA_USED$replicate <- floor((DATA_USED$replicate-0.1)/size_block) +1   #fake block to make larger data
...
RT_MEAN_SD_PLOT <- par_mean_sd_plotter(df_par_se_long =  long_par_formatter(df_par = DF_RT, df_par_se = DF_RT_SE), df_par_eq= NA, trueParameters = FALSE,  num_col=S)
